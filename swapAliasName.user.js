// ==UserScript==
// @name        Stash Performer Edit Panel, swap name with alias buttons
// @namespace     https://github.com/elkorol/stash-userscripts
...
(function () {
  "use strict";
  const { stash, Stash, waitForElementClass } = unsafeWindow.stash;
  ...
  function swapTextOld(nameInputElem, aliasGroupInputElem) {
    let nameSwapText;
    if (nameInputElem) {
      nameSwapText = nameInputElem.value;
    }
    let aliasSwapText;
    let aliasInputElem;
    if (aliasGroupInputElem) {
      const aliasInputElem = aliasGroupInputElem.querySelector("input");
      if (aliasInputElem) {
        aliasSwapText = aliasInputElem.value;
      }
    }
  ...
  stash.addEventListener("page:performer:details", function () {
    waitForElementClass("form-group", function () {
      ...
      if (nameElement) {
        const nameGroupElement = nameElement.closest("div.form-group");

        if (nameGroupElement) {
          nameInputElem = nameGroupElement.querySelector("input");
        }
      }

      const labelElement = document.evaluate(
        '//label[@for="aliases"]',
        document,
        null,
        XPathResult.FIRST_ORDERED_NODE_TYPE,
      ...
      if (labelElement) {
        const formGroupElement = labelElement.closest("div.form-group");
        if (formGroupElement) {
          const inputGroupDivs =
            formGroupElement.querySelectorAll("div.input-group");
